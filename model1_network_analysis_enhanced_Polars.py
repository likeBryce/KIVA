...
class SKUAssociationAnalyzer:
    ...
    def load_data(self):
        ...
        time1 = time.time()
        try:
            self.df = pl.read_excel(self.data_path)
            self.df = self.df.with_columns([
                pl.col('date_crt').alias('date'),
                pl.col('mat_code').cast(pl.Utf8)
            ])
    ...
    def process_orders(self):
        ...
        # 按订单分组生成SKU集合
        orders_grouped = self.df.group_by('order_num').agg(
            pl.col('mat_code').unique().alias('skus')
        )
        
        # 转换为Python对象以便后续处理
        self.orders = orders_grouped.to_pandas()
        self.orders['skus'] = self.orders['skus'].apply(set)
    ...
    def analyze_sales_characteristics(self):
        ...
        skus_sale = self.df.with_columns(
            pl.col('sa_qty_bum').cast(pl.Int64)
        ).group_by('mat_code').agg(
            pl.col('sa_qty_bum').sum().alias('num')
        )
        sku_sales = dict(zip(skus_sale['mat_code'].to_list(), skus_sale['num'].to_list()))
        ...
        # 按天分组统计每天销售的商品集合
        daily_skus = self.df.group_by(
            pl.col('date').dt.date()
        ).agg(
            pl.col('mat_code').unique().alias('mat_codes')
        )
        daily_skus_pandas = daily_skus.to_pandas()
        daily_skus_pandas['mat_codes'] = daily_skus_pandas['mat_codes'].apply(set)
        
        for sku in self.skus:
            sku_hot_sale_inx[sku] = sum(1 for skus in daily_skus_pandas['mat_codes'] if sku in skus)
    ...
    def save_analysis_to_json(self, json_path="d:\\Mycode\\KIVA\\result\\problem1\\analysis_results.json"):
        """保存分析结果到JSON文件"""
        import json
        import numpy as np
        ...
        json_data = {
            'skus': self.skus,
            'association_matrix': self.association_matrix.tolist() if isinstance(self.association_matrix, np.ndarray) else self.association_matrix,
            'analysis_results': {
                ...
                'network_nodes': self.analysis_results.get('network_nodes', 0),
                'network_edges': self.analysis_results.get('network_edges', 0),
                'network_density': self.analysis_results.get('network_density', 0)
            }
        }
        ...
        print(f"分析结果已保存到JSON文件: {json_path}")
    ...
    def run_complete_analysis(self):
        ...
        df_association_matrix = pl.DataFrame(self.association_matrix) # 打印关联矩阵
        df_association_matrix.write_excel(r"D:\Mycode\KIVA\result\problem1\association_matrix.xlsx")
        # 4. 构建网络
        self.build_network()
